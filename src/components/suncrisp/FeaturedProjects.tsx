...
const FeaturedProjects = ({
  ...
  const loopedItems = variant === 'scroll' ? [...featuredItems, ...featuredItems, ...featuredItems] : featuredItems;
  ...
  if (variant === 'scroll') {
    return <section className="py-6 sm:py-8 md:py-10 bg-background relative overflow-hidden">
        ...
        <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
          <Reveal>
            <div className="flex items-end justify-between mb-4 sm:mb-6">
              ...
              {onViewAll && <button onClick={onViewAll} className="group flex items-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-primary transition-colors min-h-[44px] px-2 pt-[13px] ml-[4px]">
                  View All
          ...
          <div className="relative">
            {/* Left scroll button */}
            <button onClick={() => scroll('left')} className="absolute left-0 top-1/3 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 backdrop-blur-md border border-primary/30 shadow-lg hover:bg-primary hover:border-primary text-primary hover:text-primary-foreground transition-all duration-300 -ml-2 sm:ml-0 group flex items-center justify-center min-w-[44px] min-h-[44px]" aria-label="Scroll left">
              <ChevronLeft className="w-5 h-5 group-hover:-translate-x-0.5 transition-transform" />
            </button>
            
            {/* Right scroll button */}
            <button onClick={() => scroll('right')} className="absolute right-0 top-1/3 -translate-y-1/2 z-10 w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 backdrop-blur-md border border-primary/30 shadow-lg hover:bg-primary hover:border-primary text-primary hover:text-primary-foreground transition-all duration-300 -mr-2 sm:mr-0 group flex items-center justify-center min-w-[44px] min-h-[44px]" aria-label="Scroll right">
              <ChevronRight className="w-5 h-5 group-hover:translate-x-0.5 transition-transform" />
            </button>

            <div ref={scrollRef} onScroll={handleScroll} className="overflow-x-auto pb-4 px-6 sm:px-8 scrollbar-hide scroll-smooth -mx-4 sm:mx-0">
              <div className="flex gap-3 sm:gap-4" style={{
              minWidth: 'max-content'
            }}>
                {loopedItems.map((item, index) => <Reveal key={`${item.id}-${index}`} delay={index % featuredItems.length * 0.1}>
                    <article className="group cursor-pointer w-56 sm:w-64 md:w-72 flex-shrink-0" onClick={() => onItemClick?.(item)}>
                      ...
                      <div className="space-y-0.5 sm:space-y-1">
                        ...
                        {item.location && <p className="text-[10px] sm:text-xs text-muted-foreground line-clamp-1">
                            {item.location}
                          </p>}
                      </div>
                    </article>
  ...
  return <section className="py-8 sm:py-10 md:py-12 bg-background relative overflow-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        <Reveal>
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between mb-6 md:mb-8">
            ...
            {onViewAll && <button onClick={onViewAll} className="mt-3 group flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-primary transition-colors min-h-[44px] sm:mt-px my-0 py-px pt-[7px] ml-[17px]">
                View All Projects
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
          {featuredItems.map((item, index) => <Reveal key={item.id} delay={index * 0.15}>
              <article className="group cursor-pointer focus:outline-none" onClick={() => onItemClick?.(item)} onTouchEnd={e => {

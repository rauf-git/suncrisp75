      ...
      <div className="max-w-4xl mx-auto px-6 md:px-0">
        <div className="space-y-16">
          {sections.map((section, index) => (
            <Reveal key={index} delay={index * 100}>
              <div className={`${index % 2 === 1 && section.image ? 'glass-card rounded-xl p-8 md:p-10' : ''}`}>
                <div className={`flex flex-col ${section.image ? 'md:flex-row md:gap-8 md:items-start' : ''} ${index % 2 === 1 && section.image ? 'md:flex-row-reverse' : ''}`}>
                  {section.image && (
                    <div className={`mb-6 md:mb-0 md:w-1/2 md:flex-shrink-0 rounded-xl overflow-hidden shadow-elevated flex items-center justify-center ${index % 2 === 1 ? '-mx-2 md:mx-0' : ''}`}>
                      <img
                        src={section.image}
                        alt={section.heading || `Section ${index + 1}`}
                        className="w-full h-auto object-cover aspect-video"

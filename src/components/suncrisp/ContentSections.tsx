...
const ContentSections = ({
  ...
  if (useFeatureGrid) {
    return <section className="py-16 md:py-24">
        <div className="max-w-7xl mx-auto px-6 md:px-12">
          ...
          {sections.slice(0, 2).map((section, index) => <Reveal key={index} delay={index * 100}>
              <div className="max-w-4xl mx-auto mb-16">
                <div className={`flex flex-col ${section.image ? 'md:flex-row md:gap-8 md:items-start' : ''}`}>
                  {section.image && <div className="mb-6 md:mb-0 md:w-1/2 md:flex-shrink-0 rounded-xl overflow-hidden shadow-elevated">
                  ...
                  <div className={`prose prose-lg max-w-none ${section.image ? 'md:w-1/2' : ''}`}>
                    {section.heading && <h2 className="font-serif text-2xl md:text-3xl font-semibold mb-4 text-foreground">
                        {section.heading}
                      </h2>}
          ...
          {sections.length > 2 && <div className="mt-8">
              <Reveal>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {sections.slice(2).map((section, index) => <Reveal key={index + 2} delay={(index + 2) * 100}>
                    <div className="glass-card rounded-xl overflow-hidden h-full hover:shadow-elevated transition-all duration-300 group">
                      {section.image && <div className="aspect-video overflow-hidden">
                          <img src={section.image} alt={section.heading || `Section ${index + 3}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
                        </div>}
                      <div className="p-6">
                        {section.heading && <h4 className="font-serif text-lg font-semibold mb-3 text-foreground group-hover:text-primary transition-colors">
                            {section.heading}
                          </h4>}
  ...
  return <section className="py-16 md:py-24 px-0">
